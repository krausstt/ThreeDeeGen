...
| `generate.py` | Generator (SDF → Marching Cubes → mannigfaltigkeitserhaltende Vereinfachung) |
| `../../tdg/` | gemeinsame Bibliothek (SDF, Export, Checks, Vorschau) |
...
```bash
pip install -r requirements.txt   # im Repo-Root
python3 projects/mae-west-eiffel/generate.py      # ~1 min, schreibt out/mae_west_eiffel.{stl,3mf,_report.json,png}
python3 projects/mae-west-eiffel/generate.py --n-rods 20 --twist-deg 170 --out /tmp/variante   # Beispielvariante
```

Alle Parameter (`--height`, `--r-base`, `--r-waist`, `--twist-deg`, `--n-rods`, `--leg-frac`, `--d-rod-top`, `--blend`, `--min-angle`, `--voxel`, …) siehe `Params` in `generate.py`. Der Generator bricht ab, falls das Ergebnis kein einzelner wasserdichter Körper ist. Achtung: Maße sind absolute Millimeter, `--height` ändert nur die Höhe, nicht den Fußdurchmesser.

Generated by AI | Quellen: [Mae West (Kunstwerk), Wikipedia](https://de.wikipedia.org/wiki/Mae_West_(Kunstwerk)), [Werner Sobek – Mae West](https://www.wernersobek.com/de/projekte/mae-west/), [NordOstKultur München](https://nordostkultur-muenchen.de/architektur/kunst-im-oeffentlichen-raum-im-muenchner-nordosten/skulptur-mae-west/), [Eiffel Tower, Wikipedia](https://en.wikipedia.org/wiki/Eiffel_Tower), [toureiffel.paris – 300 bis 330 m](https://www.toureiffel.paris/en/news/history-and-culture/300-330-meters-story-towers-height)
